...
def teacache_wanmodel_forward(
        ...
        **kwargs,
        ...
        verify_stats = getattr(self, "_teacache_verify_stats", None)
        if verify_stats is None:
            verify_stats = {"forwards": 0, "skipped_forwards": 0, "skipped_steps": []}
            self._teacache_verify_stats = verify_stats
...
class TeaCache:
    ...
    def apply_teacache(self, model, model_type: str, rel_l1_thresh: float, start_percent: float, end_percent: float, cache_device: str):
        ...
        diffusion_model._teacache_verify_stats = {"forwards": 0, "skipped_forwards": 0, "skipped_steps": []}
        diffusion_model._teacache_verify_reported = False
        ...
        def unet_wrapper_function(model_function, kwargs):
            ...
            if current_step_index >= total_steps - 1:
                stats = getattr(diffusion_model, "_teacache_verify_stats", None)
                if stats is not None and not getattr(diffusion_model, "_teacache_verify_reported", False):
                    logging.info(
                        "TeaCache verify - skipped %s/%s steps; skipped_forwards %s/%s; skipped_steps=%s",
                    ...
                    diffusion_model._teacache_verify_reported = True
            return result
