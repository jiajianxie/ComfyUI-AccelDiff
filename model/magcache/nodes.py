...
try:
    ...
    def precompute_freqs_cis(*args, **kwargs):
        raise ImportError("precompute_freqs_cis is unavailable in this ComfyUI build.")
...
def magcache_flux_forward(
        ...
        **kwargs,
        ...
        skip_forward = False
        if enable_magcache and cur_step not in [11] and hasattr(self, 'residual_cache'):
...
def magcache_wanmodel_forward(
        ...
        **kwargs,
        ...
        cur_step = transformer_options.get("current_step")
        current_step_index = transformer_options.get("current_step_index")
        mag_ratios = transformer_options.get("mag_ratios")
        verify_stats = getattr(self, "_magcache_verify_stats", None)
        if verify_stats is None:
            verify_stats = {"forwards": 0, "skipped_forwards": 0, "skipped_steps": []}
            self._magcache_verify_stats = verify_stats
        # embeddings
        ...
        verify_stats["forwards"] += 1
        if enable_magcache and skip_forward:
            verify_stats["skipped_forwards"] += 1
            if current_step_index is not None and current_step_index not in verify_stats["skipped_steps"]:
                verify_stats["skipped_steps"].append(current_step_index)
...
class MagCache:
    ...
    def apply_magcache(self, model, model_type: str, magcache_thresh: float, retention_ratio: float, magcache_K: int, start_step: int, end_step:int):
        ...
        diffusion_model._magcache_verify_stats = {"forwards": 0, "skipped_forwards": 0, "skipped_steps": []}
        diffusion_model._magcache_verify_reported = False
        ...
        def unet_wrapper_function(model_function, kwargs):
            ...
            c["transformer_options"]["current_step_index"] = current_step_index
            with context:
                result = model_function(input, timestep, **c)

            if current_step_index >= total_infer_steps - 1:
                stats = getattr(diffusion_model, "_magcache_verify_stats", None)
                if stats is not None and not getattr(diffusion_model, "_magcache_verify_reported", False):
                    logging.info(
                        "MagCache verify - skipped %s/%s steps; skipped_forwards %s/%s; skipped_steps=%s",
                        len(stats["skipped_steps"]),
                        total_infer_steps,
                        stats["skipped_forwards"],
                        stats["forwards"],
                        stats["skipped_steps"],
                    )
                    diffusion_model._magcache_verify_reported = True
            return result
